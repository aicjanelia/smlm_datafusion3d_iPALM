...
%
% RML save intermediate data to avoid starting from scratch


function [RR, I, cost] = all2all3D_saveIntermediate(subParticles, scale, initAng, USE_GPU_GAUSSTRANSFORM, USE_GPU_EXPDIST,savename)

    N = numel(subParticles);

    % filling out the all2all registration matrix (result)
    if exist(savename,'file')
        load(savename,'result','cost')
        if ~exist('result','var')
            error('file loaded incorrectly')
        end
        w = 1;
        start = 1;
        while w
            if isempty(result{start,N})
                w = 0;
            else
                start = start+1;
            end
